...
/*   Updated: 2023/06/27 23:42:33 by eandre-f         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
EpollWrapper::EpollWrapper(size_t maxevents) : _maxevents(maxevents)
{
	...
	if (_epoll_fd == -1)
	{
		logger.error("epoll_create: " + std::string(strerror(errno)));
	}
...
int EpollWrapper::add(int fd, epoll_data_t data, uint32_t events)
{
	...
	{
		logger.error("epoll_ctl: " + std::string(strerror(errno)));
		return -1;
	}
	return 0;
}
...
int EpollWrapper::wait(int timeout)
{
	...
	if (fds == -1)
	{
		logger.error("epoll_wait: " + std::string(strerror(errno)));
	}
	return fds;
}
...
bool setNonBlocking(int fd)
{
	...
	{
		logger.error("fcntl: " + std::string(strerror(errno)));
		return false;
	}
	return true;
}
